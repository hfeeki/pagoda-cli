module Pagoda::Command
  describe Keys do
    before do
      ...
      @keys.pagoda.stub!(:user).and_return('joe')
    end
    ...
    it "adds a key from the default locations if no key filename is supplied" do
      ...
      @keys.pagoda.should_receive(:add_key).with('ssh-rsa xyz')
      @keys.add
    end
    ...
    it "list keys, trimming the hex code for better display" do
      @keys.pagoda.should_receive(:keys).and_return(["ssh-rsa  pedro@pagoda\n"])
      @keys.should_receive(:display).with('ssh-rsa AAAAB3NzaC...Fyoke4MQ== pedro@pagoda')
      @keys.list
    end
    ...
    context "key locating" do
      ...
      it "raises an exception if neither id_rsa or id_dsa were found" do
        ...
        lambda { @keys.send(:find_key) }.should raise_error(Pagoda::Command::CommandFailed)
      end
    end
  end
end
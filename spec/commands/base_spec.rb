module Pagoda::Command
  describe Base do
    before do
      ...
      @base = Base.new(@args)
      @base.stub!(:display)
      @client = mock('pagoda client', :host => 'pagoda.com')
    end

    it "initializes the pagoda client with the Auth command" do
      Pagoda::Command.should_receive(:run_internal).with('auth:client', @args)
      @base.pagoda
    end

    context "detecting the app" do
      before do
        @base.stub!(:pagoda).and_return(@client)
      end
...
[remote \"staging\"]
  url = git@pagoda.com:myapp-staging.git
  fetch = +refs/heads/*:refs/remotes/staging/*
[remote \"production\"]
  url = git@pagoda.com:myapp.git
...
[remote \"github\"]
  ...
  fetch = +refs/heads/*:refs/remotes/github/*
      ...
      it "gets the app from remotes when there's only one app" do
        @base.stub!(:git_remotes).and_return({ 'pagoda' => 'myapp' })
        @base.extract_app.should == 'myapp'
      end
      ...
      it "raises when cannot determine which app is it" do
        @base.stub!(:git_remotes).and_return({ 'staging' => 'myapp-staging', 'production' => 'myapp' })
        lambda { @base.extract_app }.should raise_error(Pagoda::Command::CommandFailed)
      end
    end
    ...
    describe "formatting" do
      it "formats app urls (http and git), displayed as output on create and other commands" do
        @base.stub!(:pagoda).and_return(mock('pagoda client', :host => 'example.com'))
